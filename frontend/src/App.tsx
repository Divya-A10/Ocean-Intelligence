...
export default function App() {
  ...
  const {
    ...
    setDay: setForecastDay,
    setIsPlaying,
    setSpeed: setPlaybackSpeed,
    runSimulation,
    reset
  } = useSimulation("bay-of-bengal", 0);
  ...
  return (
    <div className="min-h-screen bg-[#fcfcfd] text-gray-900 flex flex-col font-sans select-none antialiased relative">
      ...
      <header id="main_header_nav" className="sticky top-0 z-40 h-16 bg-white/80 backdrop-blur-md border-b border-gray-200/80 px-6 sm:px-8 flex items-center justify-between">
        <button 
          onClick={() => setActiveTab("landing")}
          className="flex items-center gap-3 text-left cursor-pointer group"
        >
          <div className="w-8 h-8 rounded-lg bg-gray-900 flex items-center justify-center text-white group-hover:bg-[#0071e3] transition">
          ...
          <button
            ...
            className={`px-4 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5 cursor-pointer shadow-sm ${
              activeTab !== "landing"
                ...
                : "bg-gray-900 hover:bg-[#0071e3] text-white"
            }`}
          >
        ...
        <main className="flex-1 p-6 md:p-8 space-y-6 overflow-y-auto max-h-[calc(100vh-64px)] bg-[#fbfbfd]">
          ...
          {activeTab === "dashboard" && (
            <div className="space-y-6 max-w-7xl mx-auto">
              ...
              {/* Information Cards synchronized with timeline */}
              ...
              {/* Active Lagrangian Particle Inspector & Alerts row */}
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                
                {/* Virtual Particle Inspector */}
