  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 70; i++) {
      ...
      list.push({
        ...
        radiusOffset: 0.5 + Math.random() * 1.5,
    ...
    for (let i = 0; i < 280; i++) {
      ...
      list.push({
        lat: activeSite.lat + latOffset,
        lng: activeSite.lng + lngOffset,
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (activeLayers.particles && particlesRef.current.length > 0) {
        ...
        // Speed multiplier adapts based on Day slider!
        const dayMultiplier = 0.5 + forecastDay * 0.35; // grows as day advances!

        particlesRef.current.forEach((p, idx) => {
          if (idx >= PARTICLE_COUNT) return;

          if (p.isGlobal) {
            // Global drift: simple westward flow near equator
            ...
            // Localized high-fidelity trajectory depending on region mechanics (drifting towards hotspots)
            if (selectedRegion === "north-pacific-gyre") {
              // Standard clockwise subtropical vortex spiraling inwards!
              p.angle += p.speed * dayMultiplier * 0.012;
              p.radiusOffset -= 0.0035 * dayMultiplier; // spiral inward
              ...
              // Compute lat / lng centered around gyre (35N, -140W)
              ...
              // Cyclonic bay swirl spiraling inwards
              p.angle -= p.speed * dayMultiplier * 0.015;
              p.radiusOffset -= 0.0025 * dayMultiplier; // spiral inward towards hotspot
              ...
              // Rapid channel linear flow from West to East directly passing through hotspot
              p.lng += p.speed * dayMultiplier * 0.12;
              // Narrow corridor confinement with slight convergence towards the center strait
              p.lat += Math.sin(p.lng * 2 + idx) * 0.04;
              p.lat = p.lat * 0.95 + currentActiveSite.lat * 0.05; // soft attraction to center lat

              // Reset once exits strait sector
              ...
              // Closed basin chaotic bouncing loop spiraling inwards
              p.angle += p.speed * dayMultiplier * 0.01;
              p.radiusOffset -= 0.002 * dayMultiplier; // spiral inward
  ...
  return (
    <div id="globe_viewport_container" className="relative flex flex-col xl:flex-row gap-6 bg-white border border-gray-200/80 rounded-2xl p-5 shadow-sm overflow-hidden">
      ...
      <div className="flex-1 relative min-h-[500px] bg-slate-900 rounded-xl border border-gray-100 flex flex-col justify-between p-5 overflow-hidden shadow-sm">
        ...
        <div className="z-10 w-full mt-auto bg-black/75 backdrop-blur-md border border-white/10 rounded-xl p-3 flex flex-col md:flex-row items-center gap-3.5 justify-between shadow-lg">
          <div className="flex items-center gap-3 w-full md:w-auto">
            <button
              onClick={() => setIsPlaying(!isPlaying)}
              className="p-2 rounded-lg bg-[#0071e3] hover:bg-[#0071e3]/90 text-white font-bold transition flex items-center justify-center cursor-pointer"
              title={isPlaying ? "Pause Simulation" : "Play Simulation"}
            >
              {isPlaying ? <Pause className="w-3.5 h-3.5 fill-current" /> : <Play className="w-3.5 h-3.5 fill-current" />}
            </button>
            <div className="flex flex-col">
              <span className="text-[8px] font-mono text-sky-300 uppercase tracking-wider font-bold">TIMELINE TRANSPORT</span>
              <span className="text-xs text-white font-medium">Day {forecastDay} representing 168-hour forecast</span>
            </div>
          </div>

          {/* Timeline Slider Indicator (Synchronized with App.tsx state) */}
          <div className="flex-1 flex items-center gap-1 w-full px-1 relative">
            <span className="text-[9px] font-mono text-gray-400 shrink-0 mr-1">Now</span>
            {[1, 2, 3, 4, 5, 6, 7].map((d) => (
              <button
                key={d}
              ...
              >
                {forecastDay === d && (
                  <span className="absolute -top-7 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-[9px] font-bold px-1.5 py-0.5 rounded shadow">
                    Day {d}
                  </span>
                )}
              </button>
